###
### Automatically generated via update_3p_localization.py script using files from 
### https://github.com/JosefNemec/PlayniteExtensions/tree/809eab47a2b3be92fad1017c6f4d92cfd7f421c6 and 
### https://github.com/JosefNemec/Playnite/tree/76e475b671f2fbb13c8cfa2a1dd85720f39f2775. 
### DO NOT MODIFY, CUZ IT MIGHT BE OVERWRITTEN DURING NEXT RUN!
###
third-party-playnite-game-install-error = Kan ikke installere spil: { $var0 }
third-party-playnite-game-uninstall-error = Kan ikke afinstallere spil: { $var0 }
third-party-playnite-disabled-title = Deaktiveret
third-party-playnite-version-label = Version
third-party-playnite-ok-label = OK
third-party-playnite-save-label = Gem
third-party-playnite-cancel-label = Annullér
third-party-playnite-yes-label = Ja
third-party-playnite-no-label = Nej
third-party-playnite-downloading-label = Downloader…
third-party-playnite-loading-label = Indlæser…
third-party-playnite-download-label = Download
third-party-playnite-progress-metadata = Henter metadata...
third-party-playnite-settings-label = Indstillinger
third-party-playnite-settings-general-label = Generelt
third-party-playnite-settings-advanced-label = Avanceret
third-party-playnite-settings-clear-cache-title = Ryd cache?
third-party-playnite-login-required = Godkendelse krævet
third-party-playnite-executable-title = Eksekverings-fil
third-party-playnite-working-dir-title = Arbejdsmappe
third-party-playnite-uninstalling = Afinstallerer
third-party-playnite-close = Luk
third-party-playnite-install-game = Installér
third-party-playnite-uninstall-game = Afinstaller
third-party-gog-settings-import-installed-label = Importer installerede spil
third-party-gog-settings-import-uninstalled-label = Importer ikke installerede spil
third-party-gog-settings-connect-account = Forbind konto
third-party-gog-login-checking = Tjekker godkendelsesstatus…
third-party-gog-logged-in = Brugeren er verificeret
third-party-gog-not-logged-in = Kræver verificering
third-party-gog-not-logged-in-error = Det lykkedes ikke at verificere brugeren
third-party-gog-authenticate-label = Verificering
third-party-gog-trouble-shooting-issues = Problemer med fejlfinding
third-party-gog-start-using-client = Begynd at bruge { $var0 } klient
third-party-gog-metadata-language-label = Metadata sprog:
